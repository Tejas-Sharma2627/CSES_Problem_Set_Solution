...
public class Tower_Of_Hanoi{
    ...
    public static void solve(){
        ...
        for(List<Integer> al: ans){
            out.println(al.get(0)+" "+al.get(1));
        }
    }
    public static void toi(int disks, int src, int dest, int temp, List<List<Integer>> ans){
        if(disks>0){
            toi(disks-1, src, temp, dest, ans);
            List<Integer> arr = new ArrayList<>();
            arr.add(src);
            arr.add(dest);
            ans.add(arr);
            toi(disks-1, temp, dest, src, ans);
        }
    }
